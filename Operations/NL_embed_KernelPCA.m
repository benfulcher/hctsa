function out = NL_embed_KernelPCA(y, tau, m, maxN)
% NL_embed_KernelPCA   Kernel PCA of a time series in an embedding space, vs. linear PCA.
%
% Reconstructs the time series as a time-delay embedding (as in NL_embed_PCA)
...
%
% ---OUTPUTS:
% The same battery of eigenvalue-spectrum statistics as NL_embed_PCA
...
% ------------------------------------------------------------------------------
%% Raw kernel PCA outputs (mirrors NL_embed_PCA's field names)
% ------------------------------------------------------------------------------
...
function stats = SUB_spectrumstats(perc, m)
	...
	% NL_embed_PCA, from a normalized (sums to 1), descending spectrum perc.
