...
toolongL = 6000;
if N > toolongL % too long
    % ++BF changed on 8/3/2010 to reduce down to first 5000. In future
    % could alter to take different subsets, or set a maximum distance range
    % allowed to make a link (using sparse), ...
	disp('Time series too long for this sort of thing... **Subsetting**');
    y = y(1:toolongL);
    N = length(y); % length of time series
% 	return	
end
...
if ~isstruct(dgaussout) && isnan(dgaussout)
    ...
    % out.dgaussk_reslbq = dgaussout.reslbq; % lbq test of randomness on residuals -- p-value
end
...
if ~isstruct(dexpout) && isnan(dexpout)
    ...
    % out.dexpk_reslbq = dexpout.reslbq; % lbq test of randomness on residuals -- p-value
end
...
if ~isstruct(dpowerout) && isnan(dpowerout)
	...
	% out.dpowerk_reslbq = dpowerout.reslbq; % lbq test of randomness on residuals -- p-value
end
