...
%
% ---NOTES:
% Surrogates are generated by DVV_surrogate_hctsa (a local subfunction below),
% hctsa's own patched version of the original toolbox's DVV_surrogate.m, fixing
% a real performance issue found 2026-08-10: the original always ran the full
% 1000-iteration IAAFT budget for every surrogate (its convergence check
% essentially never triggers -- MSE oscillates rather than converging, a known
% property of vanilla IAAFT), accounting for ~85% of this operation's runtime
% for no benefit. See DVV_surrogate_hctsa's own docstring for details.
%
...
x_surr = DVV_surrogate_hctsa(x, numSurr);
if beVocal, fprintf(1, ' Done.\n'); end
...
function Xs = DVV_surrogate_hctsa(X, Ns, max_it)
	% hctsa's own version of DVV_surrogate.m (Toolboxes/DVV_Toolbox), covering
	% only the real-valued branch NL_DVV actually uses. Two changes from the
	% original:
	%
	% (1) max_it default lowered from 1000 to 100. The IAAFT convergence check
	%     (successive-iteration MSE change < 1e-5) essentially never triggers
	%     for real data: MSE oscillates indefinitely (a well-known property of
	%     vanilla IAAFT -- see Schreiber & Schmitz) rather than settling to a
	%     fixed point. Verified directly by tracing MSE out to 200 iterations
	%     for a test series: it kept oscillating in the same band from
	%     iteration ~2 onward, never coming close to the threshold. So every
	%     surrogate was silently running the full 1000-iteration budget for no
	%     benefit, dominating ~85% of NL_DVV's total runtime. 100 iterations
	%     gives the same surrogate character at ~4-6x less cost (validated
	%     against Bonn EEG and Empirical1000).
