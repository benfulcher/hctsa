...
if nargin < 3, howtocg = 'quantile'; end
% Quantile puts an equal number into each bin

N = length(y);

if strcmp(howtocg,'updown')
   y = diff(y);
   howtocg = 'quantile'; % difference and then go by quantiles
elseif strcmp(howtocg,'embed2quadrants') || strcmp(howtocg,'embed2octants')
	% make the embedding

	if strcmp(ng,'tau'), tau = CO_fzcac(y);
	else tau = ng;
	end
...
switch howtocg
    case 'quantile'
        ...
        if any(yth==0), return; end % error -- they should all be assigned to a group
	...
	case 'embed2octants' % divide based on octants in 2-D embedding space
		% if strcmp(ng,'tau'), tau=CO_fzcac(y); end
		% m1=y(1:end-tau);
		% m2=y(1+tau:end);
		% 
		% 
